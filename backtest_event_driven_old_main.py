...
LONG_PCT = 0.20
TARGET_POSITIONS = 20
# Refresh the main factor cache on the last trading day of each period.
# Override via FACTOR_REFRESH_FREQ_OVERRIDE, e.g. M / W-FRI / D.
FACTOR_REFRESH_FREQ = os.getenv("FACTOR_REFRESH_FREQ_OVERRIDE", "M")
...
class DataLoader:
    ...
    def load_stock_data(self):
        print("[1/3] Loading HFQ daily data...")
        self.df_hfq = self._load_dir(self.hfq_dir)

        print("[2/3] Loading raw daily data...")
...
class UniverseFilter:
    ...
    def get_st_codes(self, date):
        ...
        cached = self._st_cache.get(key)
        if cached is not None:
            return cached

        st_codes = self._detect_st(key)
        self._st_cache[key] = st_codes
        return st_codes

    def is_st(self, date, code):
        return code in self.get_st_codes(date)
...
class EventTriggerEngine:
    ...
    def get_buy_candidates(self, current_holdings, n_needed, factor_cache):
        ...
        Build buy candidates from the current factor cache:
          ...
          2. Delta IVOL < 0
          3. CBMOM > 0
          4. rank by Delta IVOL ascending, then IVOL ascending
...
class EventDrivenBacktestEngine:
    def __init__(self, data_loader):
        ...
        self.factor_engine = FactorEngine(data_loader.df_hfq, data_loader.df_ff3)
    ...
    def run(self):
        ...
        refresh_dates = set(date_series.groupby(date_series.dt.to_period(FACTOR_REFRESH_FREQ)).max().tolist())
...
class PerformanceEvaluator:
    ...
    def evaluate(self):
        ...
        if self.trade_log:
            ...
            if len(sells) > 0:
                ...
                other_sells = n_sells - cbmom_sells - ivol_sells
    ...
    def plot(self, save_path=None):
        ...
        ax1.set_title(
            "Event-driven strategy: low IVOL + Delta IVOL<0 + CBMOM>0\n"
            f"{START_DATE} ~ {END_DATE} | daily scan | equal weight | Top {TARGET_POSITIONS}",
            fontsize=14,
            fontweight="bold",
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Event-driven old-main backtest")
    parser.add_argument(
        "--start",
        default=START_DATE,
...
def main():
    ...
    print("  low IVOL -> Delta IVOL < 0 -> CBMOM > 0")
    print("  buy ranking = Delta IVOL ascending | sell = CBMOM < 0 or IVOL spike")
    ...
    engine = EventDrivenBacktestEngine(loader)
    engine.run()

    evaluator = PerformanceEvaluator(engine.daily_nav, engine.trade_log, engine.daily_status)
    evaluator.evaluate()

    save_path = os.path.join(BASE_DIR, "event_driven_backtest_result.png")
    evaluator.plot(save_path=save_path)

    if engine.trade_log:
        trade_path = os.path.join(BASE_DIR, "event_driven_trades.csv")
    ...
    if engine.daily_status:
        status_path = os.path.join(BASE_DIR, "event_driven_daily_status.csv")
    ...
    if engine.daily_nav:
        nav_path = os.path.join(BASE_DIR, "event_driven_nav.csv")
